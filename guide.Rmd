...
This section further describes the Dendritic Connectivity Index (DCI). For the purposes of calculating river connectivity with the DCI a river network must be considered as a collection of river segments each linked to each other through barriers with a variable amount of passability for fish as illustrated below.

The DCI then looks at each pair of segments in the network and computes the product of the lengths of each relative to the entire length of the network and weighs this by the passability of the barrier(s) which separate them. This is done for each pair of segments and added up to get a total DCI for the network. In this way, the DCI calculates connectivity as the amount of accessible habitat from each segment. 



The subsequent sub-section explains the ways in which the DCI is modified in this analysis for increased realism.

### DCI modifications

**Habitat quality** The DCI can be further modified, as it is in this analysis, to include a weighting related to habitat quality such that connectivity between low quality habitats are not given as much importance as high quality habitats. In this analysis the weighting is based on land use but these weights can also be informed by habitat quality models for fish species of interest. 

**Dispersal threshold** One of the forms of the DCI used here incorporates a threshold distance in an attempt to measure a short term connectivity within one generation of fish. Alternatively, the original form of the DCI does not assign any distance penalty between segments such that a fish is expected to be able to disperse to any part of the watershed with equal ease. This threshold is informed by the average dispersal typical of a 4 week spawning period across a large number of freshwater fish species (14km, retrieved from Comte & Olden, 2018).
...
![](images/div_correct.png){.class width="50%" height="50%" style="display: block; margin: 0 auto"}
