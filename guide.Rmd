...
Finally the workflow presented here is developed with the objective of rendering a ranking of river reaches in a watershed based on their importance to regional connectivity in the short and long term. To this end a ranking is developed which combines multiple slightly different formulations of the DCI to try and capture the multiple facets of connectivity. In this formulation, there is a total of 3 different connectivity metrics calculated each for a different aspect of connectivity important for fish conservation: a long term measure, a short term measure which uses a dispersal threshold typical of a 4 week spawning season (14km, retrieved from Comte & Olden, 2018), and an invasive measure which measures the watershed's connectivity to the source of invasive species at the mouth of the watershed. Additionally, in an effort to ensure these connectivity prioritizations remain robust in the face of habitat restoration in the region these metrics are recalculated for 4 levels of randomized restoration across the watershed. This leaves a total of 12 connectivity metrics
...
A final piece of required data is the location of the outlet of the watershed. This data consists of a single point at the final sink of the Chateauguay watershed. In this case, the watershed has multiple outlets and this is corrected in the subsequent section about river data preparation. The outlet point will be generated at that point.

Some other optional data which are explored here are data associated with rivers which can serve as a weight during the connectivity analysis effectively biasing connectivity results to prioritize highly weighted rivers. In this case these consist of land use variables extract from the [Cadre de Reference Hydrographique de Quebec (CRHQ)](https://www.donneesquebec.ca/recherche/dataset/crhq)

## 3. Data preparation

### River data

Raw river data requires considerable editing to be imported into the `dci` package. For DCI analyses to function properly river data must follow a strict dendritic topology wherein exactly two upstream rivers join together at confluences and form single downstream rivers. Due to the resolution of spatial data and the simplifying of rivers into one dimensional lines topological errors which violate the dendritic rules can arise. Specifically these errors can either be too many rivers joining to form a single downstream river (complex confluences) or the splitting of a single river into two (divergent streams). In addition to these topological corrections there are other steps that must be taken to properly isolate the watershed of interest. The steps for cleaning and preparing the Chateauguay river data are:

1. Filter out rivers which are in US jurisdiction
2. Add river lines to ensure all individual outlets are linked to a single outlet
3. Import rivers to remove small river fragments with the help of the `dci` package and the `import_rivers` function
4. Correct topological errors with the help of the `dci` package and the `enforce_dendritic` function
5. Remove duplicate rivers
6. Import rivers a final time into the `dci` workflow with `import_rivers`

These steps are further developed and demonstrated in subsequent sub-sections.

#### Filtering out US rivers

Many rivers in the south of what is considered the Chateauguay watershed are found across the US border. These rivers should be removed from the present analysis since barrier data is only available from Canadian sources.
...
qc_extent <- read_sf("data/inputs_new.gpkg", layer = "quebec_boundary") %>%
  # Match river project
  st_transform(st_crs(grhq_raw))
...
In this case, as can be seen from the removed red section, a large part of the watershed has been removed because it is not connected to the larger central segment. In this case 
...
As shown by the maps and output message of the function this river data contains 38 complex confluences and 0 divergent confluences that must be corrected. Often solving complex confluences can lead to solving divergent confluence too. In addition, sometimes solving one can reveal another topological error. For this reason, it is recommended to correct complex confluences then re-run the `enforce_dendritic` function until all complex confluences are repaired. Then move on the divergent confluences. The following two sub-sections describe how to correct these errors in a GIS software.

##### Complex confluences

Complex confluences often arise when spatial resolution is not high enough to distinguish two closely located confluences. To correct these the end point of one of the confluence inputs should be moved to create a new divergence as illustrated. Note that the new segment which connected the two confluences should be a new line such that river intersections are correctly represented.

![](images/complex_correct.png){#id .class width=50% height=50% style="display: block; margin: 0 auto"}

##### Divergent confluences

Divergent confluences often arise when there is a large structure in the river causing a river to split or it is the result of transforming lakes to lines. To correct these one of the divergent output streams needs to be removed such that only one output remains as illustrated.

![](images/div_correct.png){#id .class width=50% height=50% style="display: block; margin: 0 auto"}
...
3. Snap barriers to river lines
4. Import barriers to the `dci` workflow with the `import_points` function
...
# 3. Snap barriers to river lines
# This step is performed in QGIS since snapping behaves strangely at times with the sf package
# To speed up this snapping a clipping operation could first be done to limit the number of points to snap
all_bar <- read_sf("data/chat_data.gpkg", layer = "all_bars")
glimpse(all_bar)

# 4. Import barriers to dci
...
The outlet data consists of a single point. Once multiple outlets of the watershed are joined together to a final sink for the watershed a point layer can be created which holds a single point at this new outlet. The only further processing required is to import the outlet to the `dci` workflow with `import_points`.
...
1. Load region 3 of the CRHQ
2. Clip the relevant CRHQ points based on the location of river lines. Unfortunately, CRHQ points often don't fall perfectly on rivers. For this reason it might be beneficial to buffer the CRHQ points and extract the relevant points this way.
...
The next step after land use variables are summarized across the watershed's rivers is to convert these values to meaningful weights. All rivers start with an equal weighting of 1 and receive penalties based on land use data. In this case the penalty scheme is as follows:

|Land use|Penalty|
|--------|-------|
|agricultural < 10|0|
|agricultural > 10 & < 30|0.2|
|agricultural > 30 & < 50|0.5|
|agricultural > 50 & < 80|0.8|
|agricultural > 80|0.9|
|urban < 10 |0.1|
|urban > 10 & < 30 |0.3|
|urban < 30 & < 50 |0.6|
...
To ensure robust rankings simple restoration scenarios were simulated. This ensures that, if the landscape changes due to restoration efforts, these recommendations will still identify the most important rivers for regional connectivity. These simulations work by randomly selecting 25%, 50%, or 75% of rivers in the region and modifying their habitat quality to 1, the highest score. In a future application of this work a more realistic scenario of restoration would be preferable. For example, using knowledge of conservation on the ground to estimate regions of high conservation and restoration activity.
...
These final rankings are a composite score of connectivity across many facets. They combine short and long term connectivity with invasion risk and help to identify important regions in the Chateauguay watershed for endemic fish connectivity. They also consider changes in the landscape through restoration simulations to ensure connectivity rankings remain robust in the face of a changing watershed.

While these metrics and rankings provide a good starting understanding of connectivity in the watershed there are many data which could serve to develop robust recommendations from this analysis. For example more intimate knowledge of both species in need of conservation and invasive species could help narrow these connectivity calculations to be more species-specific. Additionally, further information on barriers and the presence of phragmites in the area could further refine the definition of passability in the watershed. Finally, more fine tuning of the weights associated with each of these metrics could yield very different impressions of the watershed's connectivity and these should be explored in depth before concrete recommendations are delivered.