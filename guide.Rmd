...
author: "Alex Arkilanian - a.arkilanian@gmail.com"
date: "15/03/2023"
output: pdf_document
urlcolor: blue
---
...
Finally, the workflow presented here is developed with the objective of rendering a ranking of river reaches in a watershed based on their importance to regional connectivity in the short and long term. To this end, a ranking is developed which combines multiple slightly different formulations of the DCI to capture the multiple facets of connectivity. In this formulation, there is a total of 3 different connectivity metrics calculated each for a different aspect of connectivity important for fish conservation: a long term measure, a short term measure, and an invasive measure which measures the watershed's connectivity to the source of invasive species at the mouth of the watershed. Additionally, in an effort to ensure these connectivity rankings remain robust in the face of habitat restoration in the region these metrics are recalculated for 4 levels of randomized restoration across the watershed. This leaves a total of 12 connectivity metrics. The table presented below shows all of these metrics with their corresponding code used to identify the results from this analysis. 

| Scenario        | Metric         | Code       |
|-----------------|----------------|------------|
| 0% restoration  | long-term DCI  | DCI_pot_0  |
| 0% restoration  | short-term DCI | DCI_thr_0  |
| 0% restoration  | invasive DCI   | DCI_dia_0  |
| 25% restoration | long-term DCI  | DCI_pot_25 |
| 25% restoration | short-term DCI | DCI_thr_25 |
| 25% restoration | invasive DCI   | DCI_dia_25 |
| 50% restoration | long-term DCI  | DCI_pot_50 |
| 50% restoration | short-term DCI | DCI_thr_50 |
| 50% restoration | invasive DCI   | DCI_dia_50 |
| 75% restoration | long-term DCI  | DCI_pot_75 |
| 75% restoration | short-term DCI | DCI_thr_75 |
| 75% restoration | invasive DCI   | DCI_dia_75 |
...
# Read in rivers
grhq_raw <- read_sf("data/inputs.gpkg", layer = "GRHQ_full") %>%
...
```{r}
# Read in dams
dams_raw <- read_sf("data/inputs.gpkg", layer = "MELCC_dams") %>%
  # Match river data projection
  st_transform(st_crs(grhq_raw))

# Read in culverts
culverts_raw <- read_sf("data/inputs.gpkg", layer = "MTQ_culverts") %>%
  # Match river data projection
  st_transform(st_crs(grhq_raw))

# Read in waterfalls
# There are no waterfalls listed in the database for this watershed
```
...
As is obvious visually many barriers lie outside the watersheds of the study region because of the ways in which data are packaged from the different sources. In addition, the southern part of the watershed does not have any data because that region of the watershed is found in the USA. In the next section of this guide the data will be cleaned and prepared for connectivity analyses.

### Other data

A final piece of required data is the location of the outlet of the watershed. This data consists of a single downstream point connecting all the streams of the study area. In a single watershed study there is only one outlet point. In this case, the study area has multiple outlets and this is corrected in the subsequent section about river data preparation. A single unified outlet point will be generated at that point.
...
qc_extent <- read_sf("data/inputs.gpkg", layer = "QC_boundary") %>%
  # Match river project
  st_transform(st_crs(grhq_raw))
...
grhq_clipped <- read_sf("data/intermediate.gpkg", layer = "GRHQ_clipped")
...
grhq_clean <- read_sf("data/intermediate.gpkg", layer = "GRHQ_w_outlet")
...
1.  Assign types and passabilities to barriers (note that these values are arbitrary)
...
# 2. Merge barriers to a single data object
all_bar <- bind_rows(culverts, dams)
...
all_bar <- read_sf("data/intermediate.gpkg", layer = "bars_final")
glimpse(all_bar)
...
riv_final <- read_sf("data/intermediate.gpkg", layer = "GRHQ_corrected")
...
```{r}
# Read in outlet
out <- read_sf("data/intermediate.gpkg", layer = "outlet")
...
2.  Clip the CRHQ points based on the location of river lines. Unfortunately, CRHQ points often don't fall perfectly on rivers. This can be due to rounding when spatial data are transferred between R and QGIS. For this reason it might be beneficial to buffer the CRHQ points and extract the relevant points this way.
...
7.  Summarize the values held in these CRHQ points onto the river lines on which they are located. This can be done in QGIS using the `Join attributes by location (summary)` tool. In this case means are used when multiple points fall on the same river.
...
grhq_weighted <- read_sf("data/intermediate.gpkg", layer = "GRHQ_CRHQ_joined")
...
The next step after land use variables are summarized across the watershed's rivers is to convert these values to meaningful weights. Similarly to the passability values, these values are meant to illustrate the tool and are chosen arbitrarily. Urban land use is more heavily penalized than agricultural land use. All rivers start with an equal weighting of 1 and receive penalties based on land use data up to a maximum of 0.95. The value of 0.95 is used to ensure rivers don't receive a weightng of 0. In this case the penalty scheme is as follows:
...
chat_res_riv <- read_sf("data/results.gpkg", layer = "rivers_results")
chat_res_bar <- read_sf("data/results.gpkg", layer = "barriers_results")
```
...
chat_res_bar <- chat_res_bar %>%
  ...
  # Rank product is used here because it is more effective at capturing the 
  # general trend in a series of rankings than a regular mean
...
-   **Barrier passability** The barrier passabilities used in this analysis were determined based on previously published analyses. However, due to differences in species' swimming abilities, the installation of a barrier, and the environmental conditions around the barrier can all alter the passability of a barrier.

-   **Habitat quality** In this analysis land use was used as a proxy for habitat quality in the interest of simplicity and the illustration of a new methodology. While anthropogenic land use is known to alter water quality in nearby waterbodies different species can be more sensitive or tolerant to this change and, thus, we cannot expect all species to perceive the landscape equally.

-   **Migration strategies** This analysis focuses primarily on potamodromous fish and only uses the diadromous measure of the DCI for the invasive connectivity estimate. This was done for the benefit of quick illustration of what this workflow can do. However, care should be taken when considering these results for the conservation of all fish which migrate into and out of a watershed.

-   **Barrier priorities** The final ranking achieved by this workflow relates to the importance of each river segment in the overall connectivity of a watershed. This is not the same as prioritizing barriers for removal or remediation.

### Future directions

While these metrics and rankings provide a good starting understanding of connectivity in the watershed there are many data and advanced procedures which could serve to develop more robust and realistic recommendations from this analysis. Recommendations for future development on this analysis are provided below in no particular order:

-   **Barrier passability** There exist a number of methods (for example the FishXing software) to include the specific dimensions of barriers and species' specific swimming/jumping abilities to better understand how their installation can cause significant fragmentation for fish. In addition, fish ladders installed at dams were not considered here and could lead to improvements in barrier passability. Finally, there are a number of additional landscape features which can alter a culvert's passability. For example, the presence of phragmites has been shown to decrease the success of fish migration through culverts. In all, field surveys of important barriers could add realism to the connectivity estimates.

-   **Habitat quality** In a future iteration of this analysis where a group of focal species are chosen a habitat suitability model could provide a more realistic estimate of habitat quality. This would also require the integration of species location data.

-   **Migration strategies** A future iteration of this workflow could alter the suite of connectivity measures assessed to better consider a wider range of fish migration strategies.

-   **Barrier priorities** The literature on prioritizing barriers is quite rich and it can be achieved through sequentially removing each combination of barriers and recalculating the DCI each time to give each barrier a value of connectivity change. However, this can be tedious and impossible with a large watershed. The solution for large watersheds is to perform mathematical optimization considering the cost of barrier remediation against the benefit to fish connectivity.
